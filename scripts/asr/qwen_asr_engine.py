"""Qwen3-ASR engine for AutoEdit ASR workflows.
...
def get_aligner_model():
    """Load and cache only the ForcedAligner model."""
    backend = get_backend()
    cache_key = f"{backend}_aligner_only"

    if cache_key in _loaded_models:
        return _loaded_models[cache_key]

    if backend != "mlx":
        raise NotImplementedError("Standalone transcript alignment is currently supported on the MLX backend only.")

    model = _load_aligner_mlx()
...
def align_text(audio_path, transcript_text, language=None):
    """Align a provided transcript against audio and return word-level timestamps.

    This is used by the SiliconFlow ASR path: cloud ASR produces the text,
    while the local Qwen3 ForcedAligner keeps the final word timestamps
    compatible with the existing editor pipeline.
    """
    ...
    if not transcript_text:
        return ASRResult(
            language=_language_to_code(language) or "Chinese",
            text="",
            duration=duration,
            words=[],
        )
    ...
    backend = get_backend()
    if backend != "mlx":
        raise NotImplementedError("Transcript alignment for provided text is currently supported on Apple Silicon / MLX only.")

    lang_code = _language_to_code(language) or "Chinese"
    aligner_model = get_aligner_model()

    print("Running ForcedAligner on provided transcript...")
    ...
    words = _restore_punctuation(words, transcript_text)
...
def _asr_align_mlx(asr_model, aligner_model, audio_path, wav, sr, duration, language):
    ...
    # Match CUDA behavior for long audio: run ASR + alignment in chunks.
    # Use overlapping context windows so boundary timestamps stay stable.
    # MLX full-length alignment can collapse timestamps on ~5min+ inputs.
    if duration > 300:
        print("Step 1/2: Transcribing long audio in overlapping 30s chunks...")
        chunk_duration = 30.0
        ...
        for i, start in enumerate(range(0, len(wav), segment_samples)):
            keep_start = start / sr
            keep_end = min(duration, keep_start + chunk_duration)
            context_start = max(0.0, keep_start - context_duration)
            context_end = min(duration, keep_end + context_duration)

            context_start_sample = int(context_start * sr)
            context_end_sample = min(len(wav), int(context_end * sr))
            offset = context_start
            chunk_wav = np.asarray(wav[context_start_sample:context_end_sample], dtype=np.float32)
            ...
            try:
                ...
                print(
                    f"Processing MLX chunk {i + 1} "
                    f"(keep {keep_start:.1f}-{keep_end:.1f}s, context {context_start:.1f}-{context_end:.1f}s)..."
                )
                ...
                chunk_text = (asr_result.text or "").strip()
                if not chunk_text:
                    continue
                ...
                for word in chunk_words:
                    ...
                    in_range = (
                        keep_start <= midpoint < keep_end
                        or (keep_end >= duration and keep_start <= midpoint <= keep_end)
                    )
                    if in_range:
                        retained_words.append(word)

                all_words.extend(retained_words)
            finally:
                if os.path.exists(chunk_path):
                    os.remove(chunk_path)

        all_words.sort(key=lambda w: (w.start_time, w.end_time, w.text))
        full_text = "".join(w.text for w in all_words)
