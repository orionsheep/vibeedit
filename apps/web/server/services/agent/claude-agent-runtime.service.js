...
function buildSystemPrompt({
  ...
  const modeInstruction = normalizedMode === 'assemble_script'
    ? [
        ...
        '如果项目里有多个素材，先调用 get_asset_script_map 看每个素材各自讲什么、当前顺序是否合理，再继续完整读稿；不要一上来把多素材内容混成一锅再删。',
        ...
        '推荐工作流是：先确认所有素材都已转录完成；再按素材读内容并判断全局顺序；然后做一轮确定性清理（重复、停顿、口头禅）；接着重新通读当前稿件，删掉悬空碎片、补回缺失承接句，最后再做 8 项审查。',
        ...
        '如果用户只是笼统地说“执行口播拼稿”或“帮我拼口播稿”，但没有给更细约束，你应优先调用 save_snapshot，然后直接调用 auto_assemble_script 先完成一轮保守拼稿，再基于结果复查和细修；不要因为怕误删而直接 no-op。',
...
function buildUserPrompt({
  ...
  if (String(mode || '').trim() === 'assemble_script') {
    lines.push('请先理解需求，再通过工具完成修改。读操作不要过度扫描；写操作必须真实改动项目。默认是在当前已剪结果上继续局部修改，不要重来，也不要恢复完整项目，除非用户明确要求。默认保持当前顺序，不要擅自重排。当前网站没有正式的字词润色能力，所以不要改写原句；默认只做删除、恢复、去停顿和项目级管理。若用户明确要求改顺序，也只允许调整整段素材 / 整个文件之间的顺序，不要改单个视频素材内部的句子顺序、片段顺序或表达顺序。口播拼稿时不要凭几句样本或候选摘要就开始删减，必须先自己读到足够完整的当前脚本块和字幕块后再动手；如果 get_script_blocks / get_subtitle_blocks 没有限制参数，默认会直接给你全量，请优先这样读完整上下文。读完整脚本块和字幕块后，必须再调用一次 get_assemble_candidates 复查重复 take / 重复句候选，决定 no-op 前不能跳过这一步。若用户只是笼统要求执行口播拼稿，没有给更细约束，优先 save_snapshot 后直接调用 auto_assemble_script 做一轮保守拼稿，然后再复查；不要因为怕误删而直接不改。处理停顿、间隙、节奏时，必须先调用 get_pause_candidates 或直接读取 get_assemble_candidates 里的停顿候选，再把具体 gap_keys 传给 remove_pauses 做 3-8 个间隙的小批量定点删除；不要只靠一句“我已经删了停顿”就结束，也不要用 2 秒/3 秒大阈值整批扫。若用户明确要求“把所有/全部停顿都删掉”“彻底去掉停顿”“一键去停顿”，你应直接调用 remove_all_pauses() 做一次确定性的全量停顿清理。参考成熟人工剪法：优先保留产品定位/核心价值、与现有方案的差异点、实际操作流程、典型使用场景这类完整表达单元；优先删除录制准备、口头确认、重复启承句、空转解释和无信息量重复举例。参考成熟人工剪法：先删块级重复，再分两到三轮清掉 0.35-1.2 秒的明显间隙。删整句、半句、重复 take、重复表达时优先用 delete_subtitle_blocks；delete_words_by_phrase 只允许删独立短口头禅和语气词，不允许在句子中间掏词。若用户要求去口气词、去口头禅、去停顿、删间隙或压紧节奏，必须真正调用对应工具落地；即使用户没特别强调停顿，也要在语义删减后检查一次是否还残留明显长停顿，并按需调用 remove_pauses 收尾，确认 deleted_gap_count 真的增加。工具执行完成后，不要立即结束；必须重新读取当前结果做一轮强制自我审查，并逐项检查这 8 项：顺序、通顺、逻辑完整、断句衔接、重复残留、停顿/口气词处理、误删关键内容、是否还有明显可改进点；如果本轮涉及调序，还必须确认只改了素材之间的顺序，没有改单素材内部顺序。只要任一项不通过，就继续修正。最终回复里必须清楚包含这 8 项检查结论。');
    lines.push('如果项目里有多个素材，优先先调用 get_asset_script_map，看每个素材分别讲什么、当前顺序是否合理；推荐流程是：按素材理解内容 -> 如有必要先 reorder_project_assets -> 自动清理明显重复与停顿 -> 再通读当前稿件，删掉悬空碎片并补回缺失承接句。不要把多素材内容混在一起后直接粗暴删减。');
...
function hasAppliedSnapshot(appliedChanges = []) {
  return appliedChanges.some((change) => (
    didChangeApply(change) &&
    String(change.tool || change.change || '').trim() === 'save_snapshot'
  ));
}
...
async function runAutomaticAssembleMutationFallback({
  ...
  await emit({
    type: 'stage',
    step: 'assemble_recovery',
    message: '检测到本轮口播拼稿没有真正落地修改，系统正在自动补做一轮保守拼稿。',
    payload: {}
  });

  if (!hasAppliedSnapshot(appliedChanges)) {
    const snapshotArgs = {
      note: '系统自动补做口播拼稿前快照'
    };
    await emitDirectToolLifecycle(emit, 'save_snapshot', snapshotArgs);
    const snapshotResult = await executeProjectAgentToolDirect(projectId, 'save_snapshot', snapshotArgs, toolContext);
    await emitDirectToolLifecycle(emit, 'save_snapshot', snapshotArgs, snapshotResult);
  ...
  await emitDirectToolLifecycle(emit, 'auto_assemble_script', assembleArgs, assembleResult);
  if (assembleResult?.changed) {
    ...
    await emit({
      type: 'stage',
      step: 'assemble_recovery',
      message: '自动保守拼稿没有识别到可安全落地的重复块或明显停顿，本轮保持不变。',
      payload: {
        changed: false
      }
    });
  ...
  return {
    applied: Boolean(assembleResult?.changed),
