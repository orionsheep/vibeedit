...
const MUTATION_INTENT_PATTERN = /(删除|恢复|去掉|切掉|清理|压紧|压缩|精简|改写|替换|调整|排序|重排|重做|重剪|拼稿|剪(?:辑)?|去重|删停顿|去停顿|删间隙|去间隙|删空白|去空白|删口头禅|去口头禅|合并|拆分|保存快照|新建快照|恢复完整|重置|导出视频|导出xml|导出edl|导出srt|导入|生成切片|创建切片|切成.*条|切几条|拆条|出.*候选切片|切\s*\d+\s*条|来\s*\d+\s*条)/i;
